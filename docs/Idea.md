...
Another good option is to install the application to `usr.local`. When the runtime detects this directory, it will automatically bind it to `/usr/local`. Most applications should work with this, and this also gives the benefit of the `/usr` folders still being accesable from inside the container.